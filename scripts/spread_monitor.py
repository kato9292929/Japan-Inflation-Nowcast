...
誠実な設計上の注意:
  ...
  - incl(特売込)は secondary の観測添え。CGPI比較には excl のみ使う。

入力(列名は引数で上書き可):
  CGPI: date + value(YoY%想定。levelなら --cgpi-mode level)
  JIN : date + value(Jevons index level, base日=100想定)。--jin-incl-col で incl も。
...
def _cum_since_base(series, base_date):
    """level系列 → (base_used, base_idx, latest_date, latest_idx, cum_pct, window_days)"""
    s = series.dropna()
    if s.empty:
        return None
    latest_date, latest_idx = s.index[-1], float(s.iloc[-1])
    if base_date is not None:
        bd = pd.to_datetime(base_date)
        prior = s[s.index <= bd]
        base_idx = float(prior.iloc[-1]) if len(prior) else float(s.iloc[0])
        base_used = bd
    else:
        base_idx, base_used = float(s.iloc[0]), s.index[0]
    window_days = int((latest_date - base_used).days)
    cum_pct = (latest_idx / base_idx - 1) * 100.0 if base_idx else np.nan
    return base_used, base_idx, latest_date, latest_idx, cum_pct, window_days


def compute_spread(cgpi, jin, jin_incl=None, cgpi_mode="yoy", base_date=None, jin_monthly="mean"):
    # ---- upstream: CGPI 最新YoY ----
    ...
    # ---- incl secondary（特売込）と promo_absorption ----
    # excl=基調(特売除外, 主軸) / incl=特売込。特売が動くと incl <= excl になり、
    # excl_cum - incl_cum = 「特売が基調上昇を店頭でどれだけ吸収/隠しているか」(pt)。
    # 主軸はexclのまま。inclはあくまで観測用の添え。CGPI比較にはinclを使わない。
    promo = None
    if jin_incl is not None:
        r = _cum_since_base(jin_incl, base_date)
        if r is not None:
            _, _, i_date, i_idx, i_cum, _ = r
            down["incl_secondary"] = {
                "latest_index": round(i_idx, 3),
                "cum_pct_since_base": round(i_cum, 3) if not np.isnan(i_cum) else None,
                "basis": "cum_since_base_only",
            }
            if not np.isnan(cum_pct) and not np.isnan(i_cum):
                absorb = round(cum_pct - i_cum, 2)  # excl_cum - incl_cum
                promo = {
                    "excl_minus_incl_pt": absorb,
                    "as_of": str(i_date.date()),
                    "reading": ("特売が基調(excl)の上昇を店頭でどれだけ吸収/隠しているか。"
                                "プラスが大きいほど特売が転嫁を覆い隠している"
                                "（『補助金で物価が見えにくい』の民間・特売版）。"),
                    "caveat": "短窓では日次特売のオンオフでぶれる。方向の観測用。",
                }
    ...
    block = {"passthrough_gap": {
        "as_of": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
        "upstream": up, "downstream": down, "gap": gap,
    }}
    if promo is not None:
        block["passthrough_gap"]["promo_absorption"] = promo
    return block
...
def main():
    ...
    ap.add_argument("--jin-incl-col", default=None,
                    help="同じ--jinファイル内のincl(特売込)列名。指定でpromo_absorptionを追加")
    ap.add_argument("--pretty", action="store_true")
    a = ap.parse_args()

    cgpi = load_series(a.cgpi, a.cgpi_date_col, a.cgpi_col, "cgpi")
    jin = load_series(a.jin, a.jin_date_col, a.jin_col, "jin")
    jin_incl = load_series(a.jin, a.jin_date_col, a.jin_incl_col, "jin_incl") if a.jin_incl_col else None
    block = compute_spread(cgpi, jin, jin_incl=jin_incl, cgpi_mode=a.cgpi_mode,
